    ...
    for model_config_item in active_model_configs:
        ...
        try:
            ...
            if llm_run_results_data:
                ...
                similarity = classification_metrics.get('average_similarity', 0.0)
                judge_score = classification_metrics.get('average_judge_score', 0.0)
                logger.info(f"Evaluation summary for {run_identifier_log}: Average Similarity: {similarity:.4f}, Average Judge Score: {judge_score:.2f}")
                ui_utils.print_success(f"Evaluation complete for {run_identifier_log}: Average Similarity: {similarity:.4f}, Average Judge Score: {judge_score:.2f}")
            else:
...
def main():
    ...
    if all_model_runs_summary:
        ...
        header = "| {:<45} | {:<25} | {:>8} | {:>12} | {:>15} | {:>19} | {:>18} | {:>15} |".format(
            "Model", "Strategy", "Similarity", "Judge Score", "Total Time (s)", "Total Output Tokens", "Avg Ans Len", "Total Cost ($)"
        )
        logger.info(header)
        ...
        for model_id, strategy_data in sorted(all_model_runs_summary.items()): 
            for strategy, data in strategy_data.items():
                ...
                if "error" in data:    
                    ...
                    similarity = data.get('average_similarity', 0.0)
                    judge_score = data.get('average_judge_score', 0.0)
                    cost = data.get('total_cost')
                    cost_str = f"${cost:.4f}" if cost is not None and cost > 0 else ("$0.0000" if cost == 0 else "N/A")
                    row_str = "| {:<45} | {:<25} | {:>8.4f} | {:>12.2f} | {:>15} | {:>19} | {:>18} | {:>15} |".format(
                        model_disp, strategy_disp, similarity, judge_score, total_time_str, token_str, ans_len_str, cost_str
                    )
                logger.info(row_str)
