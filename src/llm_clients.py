...
def get_llm_response(prompt: str, model_config: dict, is_json_response_expected: bool = False) -> dict | None:
    ...
    Args:
        ...
        is_json_response_expected: If True, attempts to parse the entire response as JSON.
                                   If False (default), returns the raw essay response.

    Returns:
        A dictionary containing {'response_content': <parsed_response_or_essay>,
        ...
        otherwise it will be the raw essay response.
    """
    ...
    if is_json_response_expected:
        try:
            ...
            return {
                "response_json": {"error": "JSON parsing failed"},
                "response_content": response_text_for_error,
        ...
        # For essay responses, check for refusals and return the raw text
        if response_text_for_error:
            refusal_phrases = [
                "cannot answer",
                "no options",
            ...
            for phrase in refusal_phrases:
                if phrase.lower() in response_text_for_error.lower():
                    logger.warning(f"Refusal phrase '{phrase}' detected in response for {config_id}. Response: '{response_text_for_error[:150]}...'")
            ...
            if refusal_detected:
                ...
                parsed_content_for_return["error"] = f"Model refused to answer: {refusal_reason}"
        else:
            logger.error(f"Empty or no usable response text received for {config_id}.")
            parsed_content_for_return = {
                "essay": "",
                "is_refusal": True,
